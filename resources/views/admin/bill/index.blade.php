   ...
   <div class="card pt-4 px-4">
      ...
      <div>
         ...
         <div class="d-flex justify-content-between" style='margin: 12px 0'>
            <div >
               <form action="{{route('search-bill')}}" method="get" class="form-inline my-2 my-lg-0">
                  @csrf
         ...
         <table class="table table-hover mt-2">
            ...
            @foreach ($bills as $item)
                <tr>
                  ...
                  <td>{{ $item->status }}</td>
                  <td class="d-flex">
                     <a href="{{route('bills.show', $item->id)}}" class="btn btn-primary" style="margin-right: 8px">Xem</a>
                     <form action="{{route('bills.update', $item->id)}}" method="post">
                        @csrf
                        @method('put')
                        <button class="btn btn-warning" style="margin-right: 8px">Duyệt</button>
                     </form>
                     <form action="{{route('bills.destroy', $item->id)}}" method="post" id="form-delete{{$item->id}}">
                        @csrf
                        @method('delete')
